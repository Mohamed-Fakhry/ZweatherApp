...
public class HomePresenter extends BasePresenter implements HomeContract.Actions {
    ...
    @Override
    public void loadCityList(final String ids) {
        Call<ListOfIDsResponse> call = weatherEndpoints.getWeatherItemsById(WEATHER_API_KEY, WEATHER_UNIT_CELICUS,
                ids);

        call.enqueue(new Callback<ListOfIDsResponse>() {
            @Override
            public void onResponse(Call<ListOfIDsResponse> call, Response<ListOfIDsResponse> response) {
                if (response.body() != null)
                    mView.addWeatherItems(response.body().getList());

            }

            @Override
            public void onFailure(Call<ListOfIDsResponse> call, Throwable t) {
                mView.showToast(t.getMessage());
            }
        });
    }
